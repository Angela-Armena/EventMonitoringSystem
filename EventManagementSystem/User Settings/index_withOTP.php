...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($buttonClicked == "btnCreate")
        {
            ...
            $retypePassword = $_POST['passwordRetype'];
        ...
        {
            $email = $_POST['emailAddress'];
            $oldPassword = $_POST['passwordCurrent'];
            $password = $_POST['password'];
            $retypePassword = $_POST['passwordRetype'];

            $_SESSION['userEmail'] = $email;

            $query = "SELECT Email FROM facilitatorinfo WHERE Email = '$email'";
            $result = mysqli_query($conn, $query);
            ...
            if ($result)
            {
                if (mysqli_num_rows($result) > 0)
                {
                    $query = "SELECT * FROM facilitatorinfo WHERE Email = '$email' AND Password = '$oldPassword'";
                    $result = mysqli_query($conn, $query);
        ...
        if ($buttonClicked == "btnDisable")
        {
            ...
            if ($result)
            {
                if (mysqli_num_rows($result) > 0)
                {
                    $query = "SELECT * FROM facilitatorinfo WHERE Email = '$email' AND Password = '$password'";
                    $result = mysqli_query($conn, $query);

                    if (mysqli_num_rows($result) > 0)
                    {
                        $query = "DELETE FROM facilitatorinfo WHERE Email = '$email' AND Password = '$password';";
                        $result = mysqli_query($conn, $query);
    ...
    <div id="modalVerifyOTP" class="modal">
        ...
        <div class="modal-content">
            <span class="fa fa-times btnExit"></span>
            <form method="post" class = "centerDiv">
                <h1 class="cardTitle">Email Verification</h1>
    ...
    <?php
        if ($sendVerification)
        {
            ...
            $mail->Username='angelaarmena@gmail.com';
            $mail->Password='lol8234753';

            $mail->setFrom('angelaarmena@gmail.com', 'OTP Verification');
            $mail->addAddress($email);

            $mail->isHTML(true);
            $mail->Subject="Your OTP";
            $mail->Body="<p>Dear user, </p> <h3>Your OTP is $generatedOTP <br></h3>
            <br><br>
            <p>Thank you,</p>
            <b>AdU-EMS</b>";
            ...
            if ($buttonClicked == "btnVerify")
            {
                ...
                else
                {
                    ...
                    else if ($fromModal == "Change Password")
                    {
    ...
    <script>
        ...
        var btnExitVerifyOTP = document.getElementsByClassName("btnExit")[3];
        ...
        btnExitVerifyOTP.onclick = function() { modalVerifyOTP.style.display = "none"; }
